	...
	*,
	...
	SELECT 
		...
		CASE UPPER(TRIM(prd_line))
			...
			WHEN  'T' THEN 'Touring'
			ELSE 'n/a'
	END AS prd_line, -- Map prodcut line code to descriptive values
	...
	END AS sls_order_dt, -- data type casting

	CASE WHEN sls_ship_dt= 0 OR LEN(sls_ship_dt) !=8 THEN NULL
		ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
	END AS sls_ship_dt, -- data type casting


	CASE WHEN sls_due_dt= 0 OR LEN(sls_due_dt) !=8 THEN NULL
		ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
	END AS sls_due_dt, -- data type casting
	...
	CASE
		WHEN UPPER(
			LTRIM(RTRIM(REPLACE(REPLACE(gen, CHAR(13), ''), CHAR(10), '')))
		) IN ('M', 'MALE') THEN 'Male'
	...
	CASE
		WHEN UPPER(
			LTRIM(RTRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), '')))
